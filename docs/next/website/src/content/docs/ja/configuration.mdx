...
macOS、Windows、および Fcitx 5 を使用する Linux では、プレフィックスコマンドとプレフィックスから起動したモードがアクティブな間、Herdr が ASCII 対応の入力ソースへ一時的に切り替えるようにできます:
...
macOS ではその時点の ASCII 対応キーボードレイアウトへ切り替えます。Windows では IME を英語（ASCII）入力へ切り替えます。Linux では手元のクライアントが `fcitx5-remote` を使い、Fcitx 5 を一時的に無効にします。ターミナル入力に戻るかテキストフィールドに入ると、Herdr は以前の入力ソースを復元します。もともと無効だった Fcitx は無効のままです。Linux ではクライアントの PATH に `fcitx5-remote` があり、デスクトップのセッションバスにアクセスできる必要があります。コントローラーが存在しない場合や応答しない場合も通常の入力は継続できます。この設定は他のプラットフォームには影響しません。
